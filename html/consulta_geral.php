...
$consulta = "	FROM (SELECT *, ROUND(tracao + sus + velocidade + aceleracao + slalom, 2) as dinamicasSum, (tracao + sus + velocidade + aceleracao + slalom) as dinamicas	
	...
	FROM (SELECT *, 	IF(SlaTempC2 IS NULL, SlaTempC1,IF(SlaTempC1 < SlaTempC2, SlaTempC1, SlaTempC2)) as resSla,
						...
						(sus_1_temp ) as SusTempC1,
						(sus_2_temp ) as SusTempC2
	...
	";
if($pg == 'ger'){$dados = DB::queryRaw("SELECT num, escola, equipe, seguranca, conforto, estaticasSum, enduro, dinamicasSum,
		IF(ROUND(seguranca + conforto + estaticas + enduro + dinamicas+ 0.00001,2) < 0, 0, ROUND(seguranca + conforto + estaticas + enduro + dinamicas+ 0.00001,2)) as total, velocidade, aceleracao, slalom, sus, tracao".$consulta." order by total DESC, num ASC");
