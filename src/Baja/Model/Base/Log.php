...
abstract class Log implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Baja\\Model\\Map\\LogTableMap';
    ...
    /**
     * The value for the pagina field.
     *
     * @var        string
     */
    protected $pagina;
    ...
    /**
     * Initializes internal state of Baja\Model\Base\Log object.
     * @see applyDefaults()
     */
    ...
    public function setUser($v)
    {
        ...
        if ($this->user !== $v) {
            $this->user = $v;
            $this->modifiedColumns[LogTableMap::COL_USER] = true;
        }
    ...
    public function setPagina($v)
    {
        ...
        if ($this->pagina !== $v) {
            $this->pagina = $v;
            $this->modifiedColumns[LogTableMap::COL_PAGINA] = true;
        }
    ...
    public function setEquipe($v)
    {
        ...
        if ($this->equipe !== $v) {
            $this->equipe = $v;
            $this->modifiedColumns[LogTableMap::COL_EQUIPE] = true;
        }
    ...
    public function setDados($v)
    {
        ...
        if ($this->dados !== $v) {
            $this->dados = $v;
            $this->modifiedColumns[LogTableMap::COL_DADOS] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : LogTableMap::translateFieldName('User', TableMap::TYPE_PHPNAME, $indexType)];
            $this->user = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : LogTableMap::translateFieldName('Pagina', TableMap::TYPE_PHPNAME, $indexType)];
            $this->pagina = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : LogTableMap::translateFieldName('Equipe', TableMap::TYPE_PHPNAME, $indexType)];
            $this->equipe = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : LogTableMap::translateFieldName('Dados', TableMap::TYPE_PHPNAME, $indexType)];
            $this->dados = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : LogTableMap::translateFieldName('Data', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Baja\\Model\\Log'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(LogTableMap::COL_USER)) {
            $modifiedColumns[':p' . $index++]  = 'user';
        }
        if ($this->isColumnModified(LogTableMap::COL_PAGINA)) {
            $modifiedColumns[':p' . $index++]  = 'pagina';
        }
        if ($this->isColumnModified(LogTableMap::COL_EQUIPE)) {
            $modifiedColumns[':p' . $index++]  = 'equipe';
        }
        if ($this->isColumnModified(LogTableMap::COL_DADOS)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'user':
                        ...
                        break;
                    case 'pagina':
                        $stmt->bindValue($identifier, $this->pagina, PDO::PARAM_STR);
                        break;
                    case 'equipe':
                        $stmt->bindValue($identifier, $this->equipe, PDO::PARAM_INT);
                        break;
                    case 'dados':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array())
    {
        ...
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getUser(),
            $keys[2] => $this->getPagina(),
            $keys[3] => $this->getEquipe(),
            $keys[4] => $this->getDados(),
            $keys[5] => $this->getData(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setPagina($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setEquipe($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setDados($arr[$keys[4]]);
        }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Baja\Model\Log The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(LogTableMap::COL_USER)) {
            $criteria->add(LogTableMap::COL_USER, $this->user);
        }
        if ($this->isColumnModified(LogTableMap::COL_PAGINA)) {
            $criteria->add(LogTableMap::COL_PAGINA, $this->pagina);
        }
        if ($this->isColumnModified(LogTableMap::COL_EQUIPE)) {
            $criteria->add(LogTableMap::COL_EQUIPE, $this->equipe);
        }
        if ($this->isColumnModified(LogTableMap::COL_DADOS)) {
            $criteria->add(LogTableMap::COL_DADOS, $this->dados);
        }
        if ($this->isColumnModified(LogTableMap::COL_DATA)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Baja\Model\Log (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setPagina($this->getPagina());
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Baja\Model\Log Clone of current object.
     * @throws PropelException
     */
